        ...
        <table class="table">
            ...
            <tbody align="center">
                ...
                if ($_POST) {
                    $P = $_POST["pesquisar"];
                    if (!empty($P)) {
                        $A = "SELECT * FROM regiao WHERE regiao_end =  '" . $P . "' ORDER BY id DESC";
                    } else {
                        $A = "SELECT * FROM regiao ORDER BY id DESC";
                    }
                } else {
                    $A = "SELECT * FROM regiao ORDER BY id DESC";
                }
                    ...
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>
                                ...
                                <td>{$rs->regiao_num}</td>
                                <td>{$rs->regiao_cid}</td>
                                <td>{$rs->regiao_compl}</td>
                                <td>{$rs->regiao_uf}</td>
                                <td>{$rs->regiao_bairro}</td>
                                <td><button type='button' class='btn btn-secondary' style='height:30px; font-size:x-small;'><a href='alterarregiao.php?cod={$rs->id}' style='color:white'>Alterar</a></button>
                                <button type='button' class='btn text-white' style='height:30px ; font-size:x-small ;background-color: rgba(105, 10, 10, 0.74);'><a href='regiaoExcluir.php?cod={$rs->id}' style='color:white'>Deletar</a></button>
                                </td>
                                </tr>";
                        }
                    }
