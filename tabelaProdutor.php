...
<body>
    ...
    <div class="boxtable" style="margin-top: 10px;">
        <table class="table">
            ...
            <tbody align="center">
                ...
                if ($_POST) {
                    $P = $_POST["pesquisar"];
                    if (!empty($P)) {
                        $A = "SELECT * FROM produtor WHERE pro_nome =  '" . $P . "' ORDER BY id DESC";
                    } else {
                        $A = "SELECT * FROM produtor ORDER BY id DESC";
                    }
                } else {
                    $A = "SELECT * FROM produtor ORDER BY id DESC";
                }
                    ...
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>
                                ...
                                <td>{$rs->pro_Cpf}</td>
                                <td>
                                <button type='button' class='btn text-white' style='height:30px ; font-size:x-small ;background-color:grey;'><a href='alterarprodutor.php?cod={$rs->id}' style='color:white'>Alterar</a></button>

                                <button type='button' class='btn text-white' style='height:30px ; font-size:x-small ;background-color: rgba(105, 10, 10, 0.74);'><a href='produtorExcluir.php?cod={$rs->id}' style='color:white'>Deletar</a></button>
                                </td>
                                </tr>";
                        }
                    }
